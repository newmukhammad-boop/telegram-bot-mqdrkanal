...
# ─── Admin: receive material upload via bot ─────
# (Admin paneldan yuklash tavsiya etiladi, lekin
#  bot orqali ham yuklash mumkin)
...
async def receive_material(msg: types.Message):
    ...
    add_material(mat_id, {"file_id": file_id, "type": ftype, "caption": caption, "name": f"Material {mat_id}"})
...
async def list_materials(msg: types.Message):
    ...
    for mid, m in mats.items():
        ...
        lines.append(f"• <b>{m.get('name', mid)}</b>\n  🔗 {link}")
...
async def delete_mat(msg: types.Message):
    ...
    if delete_material(mat_id):
        await msg.answer(f"🗑 Material <code>{mat_id}</code> o'chirildi.", parse_mode="HTML")
    else:
